...
// TestConfigParser_ParseFlags_Normal はConfigParser.ParseFlagsの正常系テスト
func TestConfigParser_ParseFlags_Normal(t *testing.T) {
	...
	// ParseFlagsを呼び出す前に、フラグを設定する必要がある
	var config Config
	mockFlagParser.StringVar(&config.Path, "path", "", "対象パス（必須）")
	mockFlagParser.BoolVar(&config.Recursive, "recursive", true, "再帰検索（デフォルト: true）")
	mockFlagParser.StringVar(&config.OutputFormat, "output-format", "text", "出力形式（text/json、デフォルト: text）")

	// モックの設定値を設定
	mockFlagParser.SetStringValue("path", testFile)
	mockFlagParser.SetBoolValue("recursive", false)
	mockFlagParser.SetStringValue("output-format", "json")
...
// TestConfigParser_ParseFlags_ParseError はParseエラーのテスト
func TestConfigParser_ParseFlags_ParseError(t *testing.T) {
	...
	if err != parseError {
		t.Errorf("エラーが期待値と異なります。期待値: %v, 実際: %v", parseError, err)
	}
}

// TestConfigParser_ParseFlags_ValidationError はバリデーションエラーのテスト
func TestConfigParser_ParseFlags_ValidationError(t *testing.T) {
	...
	// 無効なパスを設定（空のパス）
	mockFlagParser.SetStringValue("path", "")
	mockFlagParser.SetBoolValue("recursive", true)
	mockFlagParser.SetStringValue("output-format", "text")
...
func TestParseFlags_Normal(t *testing.T) {
	...
	os.Args = []string{"test", "-path=" + testFile, "-recursive=false", "-output-format=json"}
	...
	// Assert
	if err != nil {
		t.Fatalf("ParseFlagsでエラーが発生しました: %v", err)
	}
	if config == nil {
		t.Fatal("configがnilです")
	}
...
// TestValidateConfig_JsonOutputFormat はJSON出力形式のバリデーションテスト
func TestValidateConfig_JsonOutputFormat(t *testing.T) {
	// Arrange
	tc := &TestConfigParser{}
	tc.setupTestEnvironment(t)
	defer tc.teardownTestEnvironment(t)
...
// TestValidateConfig_TextOutputFormat はテキスト出力形式のバリデーションテスト
func TestValidateConfig_TextOutputFormat(t *testing.T) {
	// Arrange
	tc := &TestConfigParser{}
	tc.setupTestEnvironment(t)
	defer tc.teardownTestEnvironment(t)
	...
	if result.OutputFormat != "text" {
		t.Errorf("OutputFormatが期待値と異なります。期待値: text, 実際: %s", result.OutputFormat)
	}
}

// TestNewConfig_JsonOutputFormat はJSON出力形式でのNewConfigテスト
func TestNewConfig_JsonOutputFormat(t *testing.T) {
	// Arrange
	tc := &TestConfigParser{}
	tc.setupTestEnvironment(t)
	defer tc.teardownTestEnvironment(t)
...
// TestConfigParser_validateConfig_Normal はConfigParser.validateConfigの正常系テスト
