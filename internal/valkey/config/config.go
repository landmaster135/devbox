...
func ParseFlagsWithParser(parser FlagParser) (*Config, error) {
	...
	parser.StringVar(&operation, "operation", "", "Valkey操作 (get-keys, get-value, get-type, set-value, delete-key, delete-keys, select-keys, get-all-values, delete-data)")
	parser.StringVar(&operation, "o", "", "操作の短縮形")

	// データ操作のパラメータ
	parser.StringVar(&key, "key", "", "単一キー")
	parser.StringVar(&key, "k", "", "キーの短縮形")
	parser.StringVar(&keys, "keys", "", "複数キー（カンマ区切り）")
	parser.StringVar(&pattern, "pattern", "", "パターン")
	parser.StringVar(&pattern, "p", "", "パターンの短縮形")
	parser.StringVar(&value, "value", "", "値")
	parser.StringVar(&value, "v", "", "値の短縮形")
	parser.BoolVar(&all, "all", false, "全件フラグ")
	parser.BoolVar(&all, "a", false, "全件フラグの短縮形")
	parser.BoolVar(&dryRun, "dry-run", false, "ドライランフラグ")
	parser.BoolVar(&dryRun, "dr", false, "ドライランフラグの短縮形")

	// 接続関連のパラメータ
	parser.StringVar(&host, "host", "localhost", "Valkeyホスト")
	parser.StringVar(&host, "h", "localhost", "ホストの短縮形")
	parser.IntVar(&port, "port", 6379, "Valkeyポート")
	parser.StringVar(&password, "password", "", "Valkeyパスワード")
	parser.StringVar(&password, "pass", "", "パスワードの短縮形")
	parser.IntVar(&database, "database", 0, "データベース番号")
	parser.IntVar(&database, "db", 0, "データベース番号の短縮形")

	parser.BoolVar(&help, "help", false, "ヘルプを表示")
	parser.BoolVar(&help, "help-flag", false, "ヘルプの短縮形")
