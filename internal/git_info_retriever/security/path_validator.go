...
type MockPathValidator struct {
	...
	ValidateArchiveFunc   func(string) (string, error)
	ValidateComponentFunc func(string) (string, error)
	ValidateURLStringFunc func(string) (string, error)
}
...
func (m *MockPathValidator) ValidateArchiveDirectory(path string) (string, error) {
	if m.ValidateArchiveFunc != nil {
		return m.ValidateArchiveFunc(path)
	}
	return path, nil
}

func (m *MockPathValidator) ValidatePathComponent(component string) (string, error) {
	if m.ValidateComponentFunc != nil {
		return m.ValidateComponentFunc(component)
	}
	return component, nil
}

func (m *MockPathValidator) ValidateURLString(rawURL string) (string, error) {
	if m.ValidateURLStringFunc != nil {
		return m.ValidateURLStringFunc(rawURL)
	}
	return rawURL, nil
}
...
func (pv *PathValidator) ValidateWorkingDirectory(path string) (string, error) {
	return pv.validatePath(path, validationOptions{
		requireDirectoryExists: true,
		requireGitRepository:   true,
	})
}

// ValidateArchiveDirectory はアーカイブディレクトリとして安全なパスか検証する
func (pv *PathValidator) ValidateArchiveDirectory(path string) (string, error) {
	return pv.validatePath(path, validationOptions{
...
// ValidatePathComponent は個別のパス要素に危険な文字が含まれていないか検証する
func (pv *PathValidator) ValidatePathComponent(component string) (string, error) {
	if component == "" {
		return "", NewSecurityError(ErrorTypeEmptyPath, component, "値が空です", "")
	}

	if len(component) > pv.maxPathLength {
		return "", NewSecurityError(ErrorTypePathTooLong, component, "値が長すぎます", fmt.Sprintf("最大: %d文字", pv.maxPathLength))
	}

	normalized, err := pv.normalizeAndValidateEncoding(component)
	if err != nil {
		return "", err
	}

	if strings.ContainsAny(normalized, "/\\") {
		return "", NewSecurityError(ErrorTypeDangerousPattern, component, "パス区切り文字は使用できません", "")
	}

	if err := pv.checkAllowedCharacters(normalized); err != nil {
		return "", err
	}

	if err := pv.checkEnhancedDangerousPatterns(normalized); err != nil {
		return "", err
	}
...
// ValidateURLString はURL文字列に危険な文字が含まれていないか検証する
func (pv *PathValidator) ValidateURLString(rawURL string) (string, error) {
	if rawURL == "" {
		return "", NewSecurityError(ErrorTypeEmptyPath, rawURL, "URLが空です", "")
	}

	normalized, err := pv.normalizeAndValidateEncoding(rawURL)
	if err != nil {
		return "", err
	}
	...
	if trimmed == "" {
		return "", NewSecurityError(ErrorTypeEmptyPath, rawURL, "URLが空白のみです", "")
	}

	if err := pv.checkAllowedCharacters(trimmed); err != nil {
		return "", err
	}

	if err := pv.checkEnhancedDangerousPatterns(trimmed); err != nil {
		return "", err
	}

	if parsed, err := url.Parse(trimmed); err != nil || parsed.Scheme == "" || parsed.Host == "" {
		return "", NewSecurityError(ErrorTypeDangerousPattern, rawURL, "無効なURL形式です", "")
	}

	return trimmed, nil
}

func (pv *PathValidator) validatePath(path string, opts validationOptions) (string, error) {
	if path == "" {
	...
	if opts.requireDirectoryExists {
		if err := pv.checkDirectoryExists(cleanPath); err != nil {
			return "", err
		}
	} else {
		if err := pv.checkDirectoryIfExists(cleanPath); err != nil {
			return "", err
		}
	}

	if opts.requireGitRepository {
...
// checkDirectoryIfExists はディレクトリが存在する場合にディレクトリであることを確認する
func (pv *PathValidator) checkDirectoryIfExists(path string) error {
