...
func TestPathValidator_ValidateArchiveDirectory(t *testing.T) {
	homeDir, err := os.UserHomeDir()
	if err != nil {
		t.Fatalf("Failed to get home directory: %v", err)
	}
	...
	validPath := "./archives"
	validated, err := validator.ValidateArchiveDirectory(validPath)
	if err != nil {
		t.Fatalf("ValidateArchiveDirectory() error = %v, want nil", err)
	}

	if !filepath.IsAbs(validated) {
		t.Errorf("ValidateArchiveDirectory() = %v, want absolute path", validated)
	}

	if !strings.Contains(validated, "archives") {
		t.Errorf("ValidateArchiveDirectory() = %v, want path containing 'archives'", validated)
	}

	// システムパス配下は拒否される
	if _, err := validator.ValidateArchiveDirectory("/tmp/malicious"); err == nil {
		t.Error("ValidateArchiveDirectory() error = nil, want error for system path")
	}
}

func TestPathValidator_ValidatePathComponent(t *testing.T) {
	validator := NewDefaultPathValidator()

	safe, err := validator.ValidatePathComponent("safe-name")
	if err != nil {
		t.Fatalf("ValidatePathComponent() error = %v, want nil", err)
	}
	if safe != "safe-name" {
		t.Errorf("ValidatePathComponent() = %v, want safe-name", safe)
	}

	if _, err := validator.ValidatePathComponent("unsafe/name"); err == nil {
		t.Error("ValidatePathComponent() error = nil, want error for path separator")
	}

	if _, err := validator.ValidatePathComponent("name;rm -rf /"); err == nil {
		t.Error("ValidatePathComponent() error = nil, want error for dangerous characters")
	}
}

func TestPathValidator_ValidateURLString(t *testing.T) {
	validator := NewDefaultPathValidator()

	url := " https://github.com/example/repo.git "
	validated, err := validator.ValidateURLString(url)
	if err != nil {
		t.Fatalf("ValidateURLString() error = %v, want nil", err)
	}
	if validated != "https://github.com/example/repo.git" {
		t.Errorf("ValidateURLString() = %v, want trimmed URL", validated)
	}

	if _, err := validator.ValidateURLString(";malicious"); err == nil {
		t.Error("ValidateURLString() error = nil, want error for dangerous URL")
	}

	if _, err := validator.ValidateURLString("https://"); err == nil {
		t.Error("ValidateURLString() error = nil, want error for invalid URL")
	}
}
