...
type MemoLister interface {
	Execute(ctx context.Context, pageSize int, pageToken string, state string, orderBy string, filter string, anyContents []string, allContents []string) (*common.ListMemosOutput, error)
}
...
func (s *Service) Execute(ctx context.Context, srcTag string, destTag string) (*common.UpdateTagOutput, error) {
	...
	pageToken := ""
	for {
		listResult, err := s.memoLister.Execute(ctx, listMemosPageSize, pageToken, "", "", filter, nil, nil)
		if err != nil {
			return nil, err
		}
		if listResult == nil {
