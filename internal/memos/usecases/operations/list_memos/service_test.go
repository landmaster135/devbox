...
func TestServiceOperationListMemos_WithContents_DedupByMemoID(t *testing.T) {
	callIndex := 0
	client := &testutil.MockHTTPClient{
		DoFunc: func(r *http.Request) (*http.Response, error) {
			callIndex++
...
func TestServiceOperationListMemos_WithContentsAndFilter_CombineConditions(t *testing.T) {
	client := &testutil.MockHTTPClient{
		DoFunc: func(r *http.Request) (*http.Response, error) {
			query := r.URL.Query()
