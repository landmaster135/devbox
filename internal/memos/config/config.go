...
func PrintUsage() {
	...
	fmt.Fprintf(os.Stderr, "        -page-size (任意), -page-token (任意), -state (任意), -order-by (任意), -filter (任意: CEL形式、created_ts/updated_ts の RFC3339 比較値は自動変換), -any-contents (任意: カンマ区切りで content.contains を複数評価)\n")
