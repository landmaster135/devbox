...
func TestService_Execute_Normal(t *testing.T) {
	...
	mockRepo.ListFilesRecursiveFunc = func(dirPath string) ([]string, error) {
		if dirPath != "/tmp/body" {
			t.Fatalf("unexpected dirPath: %s", dirPath)
		}
		return []string{
			"/tmp/body/a.txt",
			"/tmp/body/b.txt",
	...
	result, err := service.Execute(" /tmp/body ", 2)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	if !strings.Contains(result, "対象ファイル総数=3") {
		t.Fatalf("unexpected total count: %s", result)
	}
	if !strings.Contains(result, "閾値超過ファイル総数=1") {
		t.Fatalf("unexpected exceeded count: %s", result)
	}
	if !strings.Contains(result, "/tmp/body/c.txt: 3") {
		t.Fatalf("unexpected exceeded list: %s", result)
	}
	if strings.Contains(result, "/tmp/body/a.txt") || strings.Contains(result, "/tmp/body/b.txt") {
		t.Fatalf("boundary files should not be exceeded: %s", result)
	}
}

func TestService_Execute_NoExceededFiles(t *testing.T) {
	t.Parallel()
	...
	service := NewService(mockRepo)
	result, err := service.Execute("/tmp/body", 1)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	if !strings.Contains(result, "(なし)") {
		t.Fatalf("unexpected no results output: %s", result)
	}
}
...
func TestService_Execute_Error(t *testing.T) {
	...
	t.Run("negative threshold", func(t *testing.T) {
		t.Parallel()
		service := NewService(&filesystem.MockRepository{})
		_, err := service.Execute("/tmp/body", -1)
