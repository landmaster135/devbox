...
func TestHandle_InputValidation(t *testing.T) {
	service := NewService(&commandExecutor.MockRepository{})
...
func TestHandle_FFmpegNotFound(t *testing.T) {
	tempDir := t.TempDir()
	srcFile := filepath.Join(tempDir, "input.mp4")
	if err := os.WriteFile(srcFile, []byte("dummy"), 0644); err != nil {
		t.Fatalf("failed to create source file: %v", err)
...
func TestHandle_FFmpegExitError(t *testing.T) {
	tempDir := t.TempDir()
	srcFile := filepath.Join(tempDir, "input.mp4")
	if err := os.WriteFile(srcFile, []byte("dummy"), 0644); err != nil {
		t.Fatalf("failed to create source file: %v", err)
