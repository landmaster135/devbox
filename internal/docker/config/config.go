...
func ParseFlags() (*Config, error) {
	...
	fs.StringVar(&cfg.Service, "service", "", "operation=ports-into-compose または volumes-into-compose のとき更新対象となるサービス名")
...
func validate(cfg *Config) error {
	...
	default:
		return fmt.Errorf("--operation は %s, %s, %s のいずれかを指定してください", OperationEnvIntoCompose, OperationPortsIntoCompose, OperationVolumesIntoCompose)
	}
	if cfg.ComposePath == "" {
...
func PrintUsage() {
	...
	fmt.Fprintf(os.Stderr, "使用方法: %s --operation=<%s|%s|%s> [オプション]\n", executable, OperationEnvIntoCompose, OperationPortsIntoCompose, OperationVolumesIntoCompose)
	fmt.Fprintln(os.Stderr, "\nオプション:")
	fmt.Fprintf(os.Stderr, "  -operation string\n        実行する操作（%s, %s, %s）\n", OperationEnvIntoCompose, OperationPortsIntoCompose, OperationVolumesIntoCompose)
	...
	fmt.Fprintln(os.Stderr, "  -service string\n        operation=ports-into-compose または volumes-into-compose のとき更新対象となるサービス名")
}
