...
// Params はインスタンス起動コマンド生成に必要な値。
type Params struct {
	InstanceName string
	Zone         string
}

// Service は start-gce-instance operation のコマンド生成を担当する。
type Service struct{}
...
func (s *Service) Build(params Params) (string, error) {
	...
	return fmt.Sprintf(
		"gcloud compute instances start %s --zone=%s",
