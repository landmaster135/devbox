...
echo '  go run cmd/cli/web-clipper/main.go \'
echo '  --operation=patch-markdown \'
echo '  --target-title="OpenAI Blog" \'
echo '  --target-url="https://openai.com/blog" \'
echo '  --src-markdown-content=$'"'"'## 記事タイトル 要約\n\n### 見出し1\n本文\n'"'"' \'
echo '  --out-file-path=./tmp/out.md \'
echo "  --top-heading-level=2"
