...
echo \"  | オプション | デフォルト値 | 説明 |\"
echo \"  |------------|--------------|------|\"
echo \"  | `-dir` | `.` | 画像ファイルを検索するディレクトリ |\"
echo \"  | `-ext` | `jpg,jpeg,tiff,tif` | 対象の画像拡張子（カンマ区切り） |\"
echo \"  | `-props` | （全て表示） | 表示するEXIFプロパティ（カンマ区切り） |\"
echo \"  | `-r` | `false` | サブディレクトリも再帰的に検索 |\"
echo \"  | `-v` | `false` | 詳細なログを出力 |\"
