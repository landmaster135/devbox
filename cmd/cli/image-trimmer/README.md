...
| `-src` | 任意 | `.` | トリミング元ディレクトリ |
| `-out` | 必須 | - | 出力先ディレクトリ |
| `-arc` | 任意 | `./5_original_files` | アーカイブ先ディレクトリ |
...
```bash
go run ./cmd/cli/image-trimmer -out ./trimmed_images -x1 10 -y1 20 -x2 300 -y2 400
```
...
サブディレクトリも含めて処理
```bash
go run ./cmd/cli/image-trimmer -src ./images -out ./trimmed_images -r -x1 10 -y1 20 -x2 300 -y2 400
```

元ファイルを移動
```bash
go run ./cmd/cli/image-trimmer -src ./images -out ./trimmed_images -move -x1 10 -y1 20 -x2 300 -y2 400
```

カスタムサフィックスを指定
```bash
go run ./cmd/cli/image-trimmer -src ./images -out ./trimmed_images -suffix cropped -x1 10 -y1 20 -x2 300 -y2 400
```

## 注意事項

- トリミング座標は必ず `x2 > x1` かつ `y2 > y1` となるように指定してください。
- `-out` は必須です。出力先ディレクトリを明示してください。
