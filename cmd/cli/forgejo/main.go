...
func printRepos(records []usecases.RepoRecord) int {
	...
	_, _ = fmt.Fprintln(w, "name\tdescription\tis_private\thttp_url\tissues_count\tpulls_count\tforks_count\tstargazers_count\tsubscribers_count\tlanguage\tlanguages\tsize\trepo_created_at\trepo_updated_at\tis_archived\ttags")
	for _, record := range records {
		...
		_, _ = fmt.Fprintf(w, "%s\t%s\t%t\t%s\t%d\t%d\t%d\t%d\t%d\t%s\t%s\t%d\t%s\t%s\t%t\t%s\n",
			record.Name,
			record.Description,
			record.IsPrivate,
			record.HTTPURL,
