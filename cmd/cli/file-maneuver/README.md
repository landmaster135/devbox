...
複数のディレクトリから指定した拡張子やファイル名部分一致条件に合致するファイルを検索し、単一の宛先ディレクトリに移動またはコピーするCLIツールです。

## 機能

- **複数ソースディレクトリ対応**: カンマ区切りで複数のソースディレクトリを指定可能
- **複数拡張子対応**: カンマ区切りで複数の拡張子を指定可能
- **ファイル名フィルター**: `--name-contains` で部分一致するファイル名だけを対象に絞り込み可能（拡張子フィルターと併用可）
...
# ファイル名に「report」を含むファイルのみ移動（拡張子指定なし）
./file-maneuver \
  --src-dirs "/path/to/source" \
  --name-contains "report" \
  --dest-dir "/path/to/reports"
...
# 拡張子とファイル名を同時にフィルタリング
./file-maneuver \
  --src-dirs "/path/to/source" \
  --extensions "jpg,png" \
  --name-contains "holiday" \
  --dest-dir "/path/to/holiday-photos"
...
| `--extensions` | ⚠️ | 対象拡張子（カンマ区切りで複数指定可能、`--name-contains` といずれか必須） | - |
| `--name-contains` | ⚠️ | ファイル名に部分一致する文字列（`--extensions` といずれか必須、両方指定可） | - |
...
⚠️: `--extensions` と `--name-contains` の少なくとも一方は必須です。
