...
- `pkg/docker/build_image.sh`: Vite 用のビルド引数を env.yml から生成して Docker イメージを作成
- `pkg/docker/dockerize_http_dev.sh`: 本CLIで同期後、開発用に docker compose を起動
- `pkg/docker/dockerize_http.sh`: 同期 & ビルド後にイメージをアーカイブ
